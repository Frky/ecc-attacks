...
We will use notations very similar to  %TODO REFERENCE ARTICLE
. Let $d$ be the scalar, $n$ the length of $d$ in binary representation. We denote by $d[i]$ the $i^{th}$ bit of $d$, 
where $d[0]$ is the most significant bit of $d$. We denote by $Q^{(i)}$ the state of the variable $Q$ before the $i^{th}$ iteration.
...
will be performed starting from $Q'^{(i)}$. 
