...
and the possible counter-measures that lead to a more robust implementation. Section \ref{non-invasive}
...
To protect against SPA, we need that the algorithm always perform the same operations. Here is an implementation of the double-and-add not vulnerable to SPA.
...
As you can see here, the addition is always performed but the result is not used when the corresponding bit is not equal to 1. Then at each step, the amount of power consumed is the same.\\
...
in section \ref{sensible-point-algo} (Algorithm \ref{d-and-a-algo}).
The demonstration of the correctness of this attack and its complexity are not detailed here, but can be found in 
...
From each value of $j$ and $x$, we compute $Q_x^{(j)} = Q^{(n)} - [x.2^j]P$, candidate for $Q^{(i)}$. 
...
Once we have the $i$ least significant bits of $d$, we can perform the same attack on iteration $i' < i$, and use the $i$ bits of $d$ to reduce the possibilities
for $x$. Repeating this, we can find all the bits of $d$.
